...
public class JsHttpClient {
    ...
    public JsHttpResponse get(String url) {
        return executeRequest(() -> {
            WebClient.RequestBuilder<Buffer> request = client.getAbs(url);
            if (!headers.isEmpty()) {
    ...
    public JsHttpResponse post(String url, Object data) {
        return executeRequest(() -> {
            WebClient.RequestBuilder<Buffer> request = client.postAbs(url);
            if (!headers.isEmpty()) {
                request.putHeaders(headers);
            }
    ...
    public JsHttpResponse sendForm(Map<String, String> data) {
        return executeRequest(() -> {
            WebClient.RequestBuilder<Buffer> request = client.postAbs("");
            if (!headers.isEmpty()) {
                request.putHeaders(headers);
            }
    ...
    public JsHttpResponse sendMultipartForm(String url, Map<String, Object> data) {
        return executeRequest(() -> {
            ...
            if (data != null) {
                for (Map.Entry<String, Object> entry : data.entrySet()) {
                    ...
                    if (value instanceof String) {
                        ...
                        form.binaryFileUpload(key, key, "application/octet-stream", Buffer.buffer((byte[]) value));
                    } else if (value instanceof Buffer) {
                        form.binaryFileUpload(key, key, "application/octet-stream", (Buffer) value);
                    } else if (value != null) {
                        // 其他类型转换为字符串
            ...
            request.sendMultipartForm(form, null);
            return request.send();
        });
    }
    ...
    public static class JsHttpResponse {
        ...
        /**
         * 获取响应体并尝试解析为JSON对象/数组
         * 优先使用Rhino进行解析，回退使用Vert.x的JsonObject/JsonArray
        ...
        public Object json() {
            ...
            try {
                ...
                try {
                    ...
                    Callable reviver = (cx, sc, thisObj, args) -> args.length > 1 ? args[1] : args[0];
                    Object parsed = NativeJSON.parse(ctx, scope, text, reviver);
                    if (parsed != null) {
                        return Context.jsToJava(parsed, Object.class); // 转 Map/List
                    }
                } finally {
                    Context.exit();
                }
            } catch (Throwable rhinoErr) {
                log.debug("Rhino JSON.parse 失败或不可用，回退解析: " + rhinoErr.getMessage());
            }
            // 最后返回原始文本
            return text;
        }
